...
public class CamelEventSynchProducer {
	...
	//EventConfigurationService evtConfigService = EventConfigurationService.getEventConfigurationService();
	...
	public void publishEventForRouteCompletion(String serviceName, Exchange camelExchange) {
		...
		ConfigurationContext configCtx1=new ConfigurationContext(configCtx.getTenantId(),configCtx.getSiteId(),null,null);
		//Events to be fired when service completed sucessfully only
		try{	logger.debug("service name : "+serviceName+", config context : "+configCtx);
				List<Event> evtProdListSucess = evtConfigService.getEventConfigProducerForServiceSuccessCompletion(configCtx1, serviceName);
				
				//logger.info("CamelEventProducerList size in CamelEventSynchProducer : " + evtProdListSucess.size());
				//Events to be fired when service completed for all condition (Sucess or failure)
				List<Event> evtProdListAll = evtConfigService.getEventConfigProducerForServiceFailedAndSucessCompletion(configCtx1, serviceName);
				...
				if(evtProdList==null || evtProdList.isEmpty()){
					logger.debug("No Event configured for SucessServiceCompletion for Service{"+serviceName+"} and requestContext="+configCtx1);
					return;
				}		
				//else build the Event and Publish Them
				publishEventForEventProducerList(camelExchange,evtProdList);
		}catch(EventBuilderInstantiationException | InvalidEventException | EventFrameworkDispatcherException | EventFrameworkConfigurationException exp){
		}
	}//end of method
	...
	public void publishEventForFailedRouteCompletion(String serviceName, Exchange camelExchange)	 {
		...
		ConfigurationContext configCtx1=new ConfigurationContext(configCtx.getTenantId(),configCtx.getSiteId(),null,null);
	try{
			List<Event> evtProdListFailure = evtConfigService.getEventConfigProducerForServiceFailedCompletion(configCtx, serviceName);
			List<Event> evtProdListAll = evtConfigService.getEventConfigProducerForServiceFailedAndSucessCompletion(configCtx, serviceName);
			//Merge the two List into one and check for nulls
			List<Event> evtProdList=mergeEventProducerList(evtProdListFailure,evtProdListAll);
			if(evtProdList==null || evtProdList.isEmpty()){
				logger.debug("No Event configured for ServiceCompletionFailure for Service{"+serviceName+"} and requestContext="+reqCtx);
				return;
			}		
			//else build the Event and Publish Them
			publishEventForEventProducerList(camelExchange,evtProdList);
	...
	private void publishEventForEventProducerList(Exchange camelExchange,List<Event> evtProdList) throws EventBuilderInstantiationException, InvalidEventException, EventFrameworkDispatcherException{
	...
	public void publishServiceCompletionSuccessSystemEvent(RequestContext reqCtx, Exchange camelExchange) {
		...
		ConfigurationContext configCtx=new ConfigurationContext(reqCtx);
		ServiceCompletionSuccessEventBuilder builder = (ServiceCompletionSuccessEventBuilder) evtConfigService.getServiceCompletionSuccessEventBuilder(configCtx);
	...
	public void publishServiceCompletionFailureSystemEvent(RequestContext reqCtx, Exchange camelExchange) {
		...
		ServiceCompletionFailureEventBuilder builder = (ServiceCompletionFailureEventBuilder) evtConfigService.getServiceCompletionFailureEventBuilder(configCtx);
		//Event configuration not required
		ROIEvent evt = builder.buildEvent(camelExchange,null);
		IROIEventService eventService = new ROIEventService();
		try {
