...
public class DynaStoreLogTest {
	...
	/*	try {
			ipDynaStorePersistenceService.deleteDynaStoreLog();
		} catch (DynaStorePersistenceException e) {
			// TODO Auto-generated catch block
			logger.error("error delete dynastore Log");
		}*/
	}

	@Test
	public void testDynaStoreSessionCreationAndTerminationLog()
			throws InvalidNodeTreeException, ConfigPersistenceException,
			DynaStorePersistenceException {
		...
		int siteNodeId = perService.getApplicableNodeId(GenericTestConstant.TEST_TENANTID,	GenericTestConstant.TEST_SITEID);
		Assert.assertTrue(siteNodeId > 0);
		String dynastoreSessionStatus = ipDynaStorePersistenceService.getDynaStoreLog(siteNodeId, dynaStoreSession.getSessionId());
		...
		dynastoreSessionStatus = ipDynaStorePersistenceService.getDynaStoreLog(siteNodeId, dynaStoreSession.getSessionId());
